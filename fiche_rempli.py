...
class CheckedBox(Flowable):
    """Dessine un carré coché (avec un X)."""
    ...
    def draw(self):
        ...
        # Long stroke from bottom up-right
        self.canv.line(s * 0.4, m, s - m, s - m)

def CB(options, checked=None):
    """Cases à cocher. checked = liste des options cochées."""
    if checked is None:
        checked = []
    cells = []
    widths = []
...
def generate_artisan_fiche(
    ...
    qr = QrCodeWidget(
        "BROBROLI - Fiche Artisan\n"
        "Nom: KOUASSI Konan Jean-Marc\n"
        "N° Dossier: BRB-2025-00147\n"
        "Catégorie: A1+ (Certifié, 5-10 ans)\n"
        "Métier: Plombier\n"
        "Tél: +225 07 58 42 31 90\n"
    ...
    # N° Dossier
    ref = Table([
        [P("N° Dossier :", True), P("BRB-2025-00147"),
         P("Date :", True), P("05 / 03 / 2025")]
    ...
    fields = [
        field_row("Nom", "KOUASSI"), field_row("Prénoms", "Konan Jean-Marc"),
        field_row("Sexe", CB(["M", "F"], checked=["M"])),
        field_row("Date de naissance", "15/06/1988"), field_row("Lieu de naissance", "Bouaké"),
        field_row("Nationalité", "Ivoirienne"),
        field_row("N° CNI / Attestation", "CI-0088-2547-19"),
        field_row("Tél. principal", "+225 07 58 42 31 90"),
        field_row("Email", "jm.kouassi@gmail.com"),
        field_row("Commune", "Cocody"), field_row("Région", "Abidjan"),
    ]
    ...
    photo_path = "profile-pic.jpeg"
    if os.path.exists(photo_path):
        photo = Image(photo_path, width=35*mm, height=35*mm, kind='direct')
    else:
    ...
    profil_data = [
        ...
        [P("Métier principal"), CB(["Plombier", "Électricien", "Peintre", "Clim/Froid", "Autre"], checked=["Plombier"])],
        [P("Métier(s) secondaire(s)"), P("Carreleur")],
        [P("Années d'expérience"), CB(["<5 ans", "5–10 ans", "10+ ans"], checked=["5–10 ans"])],
        [P("Formation professionnelle"), CB(["CAP", "BEP", "BT", "BTS", "Licence", "Aucun"], checked=["BT"])],
        [P("Établissement de formation"), P("CFPA de Bouaké")],
        [P("Diplôme certifié ?"), CB(["Oui", "Non"], checked=["Oui"])],
        [P("N° Diplôme"), P("BT-PLB-2010-0452")],
        [P("Entreprises / chantiers majeurs"), P("SOGEA CI, Chantier résidence Ivoire")],
    ]
    ...
    cat_data = [
        ...
        [CB(["A1 (<5 ans)", "A1+ (5–10 ans)", "A1++ (10+ ans)"], checked=["A1+ (5–10 ans)"])],
    ...
    docs_checked = ["CNI / Document adimistratif", "Copie diplôme (si certifié)",
                     "Attestation d'expérience", "RIB / Mobile Money",
                     "Photo en situation de travail", "Carte CMU"]

    # 3 columns × 3 rows
    doc_cells = []
    for d in docs:
        box = CheckedBox(size=11) if d in docs_checked else CheckBox(size=11)
    ...
    equip_data = [
        ...
        [P("Outils complets adaptés au métier"), cb_cell(True), cb_cell()],
        [P("Véhicule (Moto / Tricycle / Véhicule)"), cb_cell(True), cb_cell()],
        [P("Smartphone Android"), cb_cell(True), cb_cell()],
        [P("Connexion internet régulière"), cb_cell(True), cb_cell()],
        [P("Équipe disponible"), cb_cell(), cb_cell(True)],
    ]
    ...
    zone_data = [
        [PW("Élément"), PW("Détail")],
        [P("Zone(s) d'intervention"), P("Cocody, Plateau, Marcory, Riviera")],
        [P("Disponible immédiatement"), CB(["Oui", "Non"], checked=["Oui"])],
        [P("Horaires habituels"), P("07h00 – 18h00, Lun-Sam")],
        [P("Intervention d'urgence possible"), CB(["Oui", "Non"], checked=["Oui"])],
    ]
    ...
    eng_text = (
        "Je soussigné(e) <u>Mr KOUASSI Konan Jean-Marc</u> déclare exactes les informations fournies et m'engage à :"
        "<br/><br/>"
    ...
    half = CONTENT_W / 2
    try:
        sig_img = Image("signature.png", width=60*mm, height=30*mm, kind='proportional')
    except:
        sig_img = P("KOUASSI Konan Jean-Marc")
    try:
        date_img = Image("date.png", width=70*mm, height=35*mm, kind='proportional')
    except:
        date_img = P("24-02-2026")
    sign = Table([
...
if __name__ == "__main__":
    generate_artisan_fiche(
        output_filename="FICHE_REMPLIE_KOUASSI_JEAN_MARC.pdf",
        logo_path_institutionnel="logo/Logo brobroli version institutionnelle.png",
        logo_path_digital="logo/Logo brobroli version digital.png"
    )